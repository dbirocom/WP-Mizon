...
function fastestwp_breadcrumbs() {
  ...
  if (is_home() || is_front_page()) {
    ...
    } elseif ( is_404() ) {      $a1=$_SERVER['REQUEST_URI'];	  $b1=str_replace(array('.html','-'),array('',' '),$a1);	  $b1=trim($b1,'/');	  $b1=ucwords($b1);
      echo $before . $b1 . $after;
    }
...
function fastestwp_first_image() {
  global $post, $posts;
  $first_img = get_template_directory_uri() . '/images/loading.gif';
 // ob_start();
 // ob_end_clean();
...
Adsense Themes<?php } else { ?><?php echo get_theme_option('footer_ads1'); ?><?php } ?>
<?php } 
